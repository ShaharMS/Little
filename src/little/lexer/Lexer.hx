...
class Lexer {
	...
	**/
	public static function splitBlocks1(complexTokens:Array<ComplexToken>):Array<LexerTokens> {
		...
		for (complex in complexTokens) {
			switch complex {
				case DefinitionCreationDetails(line, name, complexValue, type):
					{
						...
						var defName = name,
							defType = type,
							defValue:LexerTokens = Specifics.complexValueIntoLexerTokens(complexValue);
						tokens.push(DefinitionCreation(defName, defValue, defType));
					}
				case Assignment(line, value, assignees):
					{
						tokens.push(SetLine(line));
						final parsedValue = Specifics.complexValueIntoLexerTokens(value);
				...
				case ConditionStatement(line, type, conditionExpression, conditionBody):
					{
						tokens.push(SetLine(line));
						var condition = Specifics.complexValueIntoLexerTokens(conditionExpression);
				...
				case GenericExpression(line, exp):
					{
						...
						if (exp.startsWith(FUNCTION_RETURN)) {
							tokens.push(Return(Specifics.complexValueIntoLexerTokens(exp.replaceFirst("return", "").trim())));
						} else { // Generic cases: action calls, lone values...
							tokens.push(Specifics.complexValueIntoLexerTokens(exp));
						}
					}
			}
		}
	...
	public static function prettyPrintAst(ast:Array<LexerTokens>, ?spacingBetweenNodes:Int = 6) {
	...
	static function getTree(root:LexerTokens, prefix:Array<Int>, level:Int, last:Bool):String {
		l = level;
		var t = if (last) "└" else "├";
		var c = "├";
		var d = "───";
