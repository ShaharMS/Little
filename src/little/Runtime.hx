...
/**
 * The `Runtime` class is some sort of a bridge, connecting the interpreter of the language and you, the user.
 * 
 * It contains many functions and listeners, used to interact/listen to the interpreter's actions.
 */
class Runtime {
    
    /**
     * `onNextLine` is called **after** the interpreter successfully parses and executes a line and moves on to the next line
     * 
    ...
    /**
     * `preNextLine` is called **after** the interpreter successfully parses and executes a line, but **before** the interpreter moves on to the next line
     * 
     * @param firstParameter the line number that was just parsed
     * @param secondParameter the line that was just parsed
     */
    ...
    public static function gatherMemoryStructure():Map<String, Variable> {
